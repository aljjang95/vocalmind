...
export async function proxy(request: NextRequest) {
  ...
  const connectSrc = isDev
    ...
    : `connect-src 'self' wss://*.supabase.co wss://*.fly.dev https://*.supabase.co https://*.modal.run https://*.fly.dev https://d1pzp51pvbm36p.cloudfront.net https://api.tosspayments.com blob:`;
  const csp = [
  ...
  // 로그인 상태에서 /auth/login 접근 → /journey로 리다이렉트
