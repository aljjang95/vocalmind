"""scene_planner 단위 테스트 — Anthropic API mock."""
...
class TestPickSceneCount:
    def test_short_song(self):
        assert scene_planner._pick_scene_count(50) == 3

    def test_medium_song(self):
        assert scene_planner._pick_scene_count(90) == 4
        assert scene_planner._pick_scene_count(150) == 5

    def test_long_song(self):
        assert scene_planner._pick_scene_count(240) == 6


class TestNormalizeScenes:
    def test_adds_ids_and_clamps_duration(self):
        raw = [
            {"prompt": "a warm sunset", "duration_sec": 3.0},   # too short → 6
            {"prompt": "neon city", "duration_sec": 50.0},      # too long → 20
            {"prompt": "valid", "duration_sec": 12.5},
        ]
        out = scene_planner._normalize_scenes(raw)
        assert len(out) == 3
    ...
    def test_handles_non_numeric_duration(self):
        ...
        out = scene_planner._normalize_scenes(raw)
        assert out[0]["duration_sec"] == 12.0  # 기본값
...
class TestPlanScenesWithMock:
    def test_uses_llm_response_when_valid(self):
        ...
        with patch("services.scene_planner.ai.complete_json", return_value=fake):
            scenes = scene_planner.plan_scenes("cinematic", 120)
        assert len(scenes) == 2
        # LLM 원본 프롬프트가 포함되고 스타일 앵커가 부착됨
    ...
    def test_fallback_when_llm_returns_none(self):
        with patch("services.scene_planner.ai.complete_json", return_value=None):
            scenes = scene_planner.plan_scenes("cozy", 90)
        assert len(scenes) == 4  # 90초 → 4씬
        # fallback의 base prompt가 각 scene prompt의 접두부여야 함
        cozy_bases = scene_planner._FALLBACK_PROMPTS["cozy"]
        for s in scenes:
            # base prompt가 접두부로 포함돼야 함 (뒤에 anchor 붙음)
            assert any(s["prompt"].startswith(base) for base in cozy_bases)
        # cozy anchor 토큰이 포함됨
        assert all("Kinfolk" in s["prompt"] or "golden hour" in s["prompt"] for s in scenes)

    def test_fallback_unknown_style_uses_cinematic(self):
        scenes = scene_planner._fallback_scenes("unknown_style", 4, 12.0)
        assert len(scenes) == 4
        assert "cinematic" in scenes[0]["prompt"].lower() or "anamorphic" in scenes[0]["prompt"].lower()

    def test_fallback_when_scenes_empty_list(self):
        with patch("services.scene_planner.ai.complete_json", return_value={"scenes": []}):
            scenes = scene_planner.plan_scenes("fantasy", 120)
        # 빈 scenes → fallback
        assert len(scenes) > 0

    def test_style_anchor_attached_to_every_style(self):
        """4 스타일 모두 fallback prompt에 스타일 고유 토큰이 포함돼야 함."""
        markers = {
            "cinematic": "ARRI Alexa",
            "cozy": "Kinfolk",
            "neon_city": "Blade Runner",
            "fantasy": "Ghibli",
        }
        for style, marker in markers.items():
            scenes = scene_planner._fallback_scenes(style, 2, 12.0)
            assert all(marker in s["prompt"] for s in scenes), f"{style} missing marker {marker}"
