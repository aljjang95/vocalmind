...
export async function POST(
  ...
  // 글로벌 rate limit
  const { limited: globalLimited } = checkGlobalRateLimit({ limit: 60 });
  if (globalLimited) {
  ...
  // 분당 15회
  const { limited } = checkRateLimit(ip, { limit: 15, storeMax: 5_000 });
  if (limited) {
    return NextResponse.json(
  ...
  try {
    ...
    const response = await anthropic.messages.create({
      ...
      system: cachedSystem(COACH_FEEDBACK_SYSTEM_PROMPT),
